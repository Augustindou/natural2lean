...
class TestSeparatePropostions(unittest.TestCase):
    def test_working_examples(self):
        examples = {
            ...
            "$a$ is an even natural number, $a^2$ is divisible by $3$ and $b \\in \\mathbb{N}$": "(a : Nat) ∧ even (a) ∧ divisible (3) (a^2) ∧ (b : Nat)",
            "$a, b$ and $c$ are naturals and $\\sqrt{a} + b^2 = c^2$": "(a b c : Nat) ∧ ((a) ^ (1/2)) + b^2 = c^2",
        }
